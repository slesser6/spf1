...
int main() {
  ...
  cv::VideoCapture camL("/dev/video0"), camR("/dev/video2");

  // Check if right camera is attached
  if (!camR.isOpened()) {
    std::cout << "Could not open right camera";
    return -1;
  }

  // Check if left camera is attached
  if (!camL.isOpened()) {
    std::cout << "Could not open left camera";
    return -1;
  }

  // Creating a named window to be linked to the trackbars
  cv::namedWindow("disparity", cv::WINDOW_NORMAL);
  cv::resizeWindow("disparity", 640, 360);
